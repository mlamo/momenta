...
class ModelEmcee:
    ...
    def log_prob(self, x):
        i = 0
        norms = x[i:i+self.flux.ncomponents]
        ...
        # Prior boundaries
        for n in norms:
            if not (self.fluxnorm_range[0] <= n <= self.fluxnorm_range[1]):
                return -np.inf
        ...
        acc = np.array([[s.effective_area.get_acceptance(c, int(toy["ipix"]), self.parameters.nside) for s in self.detector.samples] for c in self.flux.components])
        ...
        loglkl += self.logprior_norm(norms)
        return loglkl
